...
let nodesArr = readLine()!.split(separator: " ").map({ Int($0)! })

let tree = BinarySearchTree(array: nodesArr)

let searchCount = Int(readLine()!)!
let searchArr = readLine()!.split(separator: " ").map({ Int($0)! })

searchArr.forEach { (value) in
	print(tree.isContain(value) ? 1 : 0)
}
